...
> Answer: When client side routing is used, the entire page does not need to refresh since the URL is changed but the request to the server is prevented. This means that because less data is processed, routing between views is generally faster. It also means that smooth transitions and animations between views are easier to implement.
